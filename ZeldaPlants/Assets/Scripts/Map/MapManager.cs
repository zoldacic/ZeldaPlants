...
namespace Assets
{
    public class MapManager : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            var textures = Resources.LoadAll("Map/Desert");
            var sprites = Resources.LoadAll<Sprite>("Map/Desert");
			var sand = Resources.Load<Sprite> ("Map/Sand");
            ...
            var map = _mapGenerator.Map;
			    for (var x = 0; x < SizeX; x++)
			    {
			        for (var y = 0; y < SizeY; y++)
			        {
			            ...
			            var go = new GameObject {layer = LayerMask.NameToLayer("MapTiles")};
			            ...
			            if (spriteNo < sprites.Length)
			            {
			                ...
			                nextPosition.x += sprite.rect.width*2*x/100f;// go.transform.localScale.x * x;
			                nextPosition.y += sprite.rect.height*2*y/100f;// go.transform.localScale.y * y;
